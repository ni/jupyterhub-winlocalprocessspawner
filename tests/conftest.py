"""Ported from JupyterHub's tests/conftest.py.
...
def ssl_tmpdir(tmpdir_factory):
    """Creates a temporary 'ssl' directory."""
...
def app(request, io_loop, ssl_tmpdir):
    ...
    mocked_app = None
    ssl_enabled = getattr(request.module, "ssl_enabled", os.environ.get("SSL_ENABLED", False))
    kwargs = dict()
    if ssl_enabled:
...
def slow_bad_spawn(app):
    ...
    with mock.patch.dict(app.tornado_settings, {"spawner_class": mocking.SlowBadSpawner}):
        yield
